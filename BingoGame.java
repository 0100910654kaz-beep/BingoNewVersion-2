...
public class BingoGame implements Serializable {
    ...
    private List<String> scanSingleCard(List<List<String>> card) {
        ...
        String missingValD2 = "";
        for (int i = 0; i < 5; i++) {
            if (!marked[i][4 - i]) { missingCountD2++; missingValD2 = card.get(i)[4 - i]; } // 安全のため下のゲッターに変更
        ...
        // もし完全にビンゴ（すべての列で穴が2つ以上空いている、かつ待ちがない状態）なら空リスト
        // ただし、すでにどこかのラインが全通しているかチェック
        ...
        if (hasBingoLine) {
            return new ArrayList<>(); // ビンゴ確定
        }
        return waitNumbers;
    }

    // ★ ビンゴ達成者の登録処理（改善ポイント：一番後ろに追加して古い順にします）
    private void addBingoPlayer(String name, int currentDrawnNumber) {
        for (PlayerResult p : bingoPlayers) {
            if (p.getPlayerName().equals(name)) return;
        }
        Date now = new Date();
        // 💡 リストの一番後ろ(add)に繋げることで、最初にビンゴした人が[0]番目（ずっと先頭）になります！
